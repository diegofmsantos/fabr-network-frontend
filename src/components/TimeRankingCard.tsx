...
export const TeamRankingCard: React.FC<TeamRankingCardProps> = ({ title, category, teams }) => {
    ...
    const formatValue = (value: string | number, title: string): string => {
        ...
        if (title.includes('(AVG)')) return numValue.toFixed(1).replace('.', ',')
        if (title.includes('(%)') || title === 'FG(%)' || title === 'XP(%)') return `${Math.round(numValue)}%`
                        ...
                        >
                            ...
                            >
                                {team.isFirst ? (
                                    ...
                                    <div className="w-full h-auto flex justify-between items-center gap-2 px-2 md:px-10 lg:px-20 xl:px-32">
