/**
 * superliga-navigation.ts — atualizado para /tabela/[divisao]/[temporada]/[subrota]
 * Substitui: src/utils/superliga-navigation.ts (frontend de exibição)
 */
...
export function getSuperligaNavigation(currentPath: string, divisao: string, temporada: string) {
  ...
  const base = `/tabela/${divisao}/${temporada}`
  const prevIndex = currentIndex - 1
  const nextIndex = currentIndex + 1

  return {
    prev: prevIndex >= 0 ? {
      path: `${base}/${SUPERLIGA_PAGES[prevIndex].path}`,
      title: SUPERLIGA_PAGES[prevIndex].title
    } : null,
    next: nextIndex < SUPERLIGA_PAGES.length ? {
      path: `${base}/${SUPERLIGA_PAGES[nextIndex].path}`,
