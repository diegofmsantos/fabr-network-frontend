      ...
      <div className="xl:ml-60 2xl:ml-[550px] absolute">
        <div className="w-full border-black bg-[#ECECEC] border-b mt-20 px-3 xl:w-[900px] md:h-14 md:pt-2 xl:ml-[170px] fixed z-50 xl:h-28 xl:pt-12 xl:mt-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <h1 className="text-[22px] min-[375px]:text-[24px] min-[375px]:h-16 min-[375px]:pt-2 min-[425px]:text-[23px] font-extrabold italic leading-[55px] 
            tracking-[-2px] text-black md:pt-0 md:h-10 md:text-3xl xl:text-4xl">SEMIFINAL NACIONAL</h1>
        ...
        <div className="mt-44 h-full mb-24 ml-3 xl:ml-20">
          <div className="flex flex-col gap-8 min-[375px]:ml-4 min-[425px]:ml-2">
            {(!semifinais || semifinais.length === 0) ? (
              <div className="text-center py-1 md:ml-40 xl:ml-96"><div className="text-black text-lg">Nenhum jogo de Semifinal Nacional configurado ainda.</div></div>
            ) : (
              semifinais.map((jogo: any, idx: number) => (
                <div key={jogo.id || idx} className="bg-white rounded-lg shadow-sm border min-[375px]:w-80 min-[425px]:w-96 md:min-w-[720px] lg:min-w-[900px] 
                lg:ml-10 xl:ml-20 overflow-hidden">
                  ...
                  <div className="p-4">
                    <div className="flex items-center justify-center gap-4">
                      ...
                      <div className="flex flex-col items-center gap-2">
                        <Image
                          src={ImageService.getTeamLogo(jogo.time2)} alt={jogo.time2}
                          width={80}
                          height={80}
                          className="rounded"
                    ...
                    <div className="flex flex-col items-center gap-2 mt-4">
                      {jogo.status && <span className={`px-3 py-1 rounded-full text-sm font-medium ${jogo.status === 'FINALIZADO' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-black'}`}>{jogo.status === 'FINALIZADO' ? 'Finalizado' : jogo.status === 'AGENDADO' ? 'Agendado' : jogo.status}</span>}
